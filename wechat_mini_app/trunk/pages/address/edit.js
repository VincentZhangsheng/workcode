...
Page({
  ...
  onLoad: function (options) {
    ...
    that.data.mode = options.mode ? options.mode : '';
  ...
  setAreaData: function (p, c, d, initFlag) {
    p = p || 0 // provinceSelIndex
    c = c || 0 // citySelIndex
    d = d || 0 // districtSelIndex
    // 设置省的数据
    var province = area['1']
    var provinceName = [];
    var provinceCode = [];
          ...
          if (that.data.mode=='select'){
            ...
            if (pages.length > 1) 
            {
              ...
              var selectedAddress = {
                ...
                mobile: data.mobile,
                mergeAdress: data.provinceName + ' ' + data.cityName + ' ' + data.districtName + ' ' + data.address,
                city: data.city
              };
              prePage.selectExpressAdress && prePage.selectExpressAdress(selectedAddress);
            }
          ...
          app.sessionRequest({
            ...
            success: function (res) {
              ...
              if (res.data.code == '200') {
                ...
                // if (app.globalData.selectedAddress.address_id == that.data.addressId)
                // {
                //   app.globalData.selectedAddress = false;
                // }
                ...
                {
                  var prePage = pages[pages.length - 2];
                  prePage.delExpressAdress && prePage.delExpressAdress(that.data.addressId);
                }
